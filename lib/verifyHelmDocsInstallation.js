...
async function findHelmDocsExec(context) {
  const logger = context.logger;
    ...
    try {
      ...
      await execa('curl', ['-sSLo', 'helm-docs.tar.gz', `https://github.com/norwoodj/helm-docs/releases/download/v${version}/helm-docs_${version}_Linux_x86_64.tar.gz`]);
      await execa('tar', ['-xzf', 'helm-docs.tar.gz', '-C', tmpDir]);
      await execa('rm', ['-f', 'helm-docs.tar.gz']);
      await execa(`${tmpDir}/helm-docs`, ['-- version']);
      return 
    } catch (error) {
